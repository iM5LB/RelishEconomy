...
Premium category-based shop GUI driven by `shop.yml` (categories/items) and `prices.yml` (sell prices). GUI layout and sounds are in `gui.yml`.
...
Some sell outputs include hover breakdowns showing exactly what was sold. See also [Sell System](SellSystem.md).
