...
## 🪙 Multi-Currency Economy • Shop • Sell GUI • Physical Currency • ATM • Natural Source Currency • Personal Vault ⭐

![Relish-Economy-Logo](https://cdn.modrinth.com/data/cached_images/9df4655d0afd67ba097405f931695951fcb513f2_0.webp)
[![M5LB Store Banner](https://github.com/iM5LB/RelishEconomy/raw/main/docs/assets/M5LBStore.png)](https://m5lb.run.place/?buy=relish-economy)
...
[![Store](https://img.shields.io/badge/Store-License%20Keys-gold?style=for-the-badge)](https://m5lb.run.place/?buy=relish-economy)
[![Donate](https://img.shields.io/badge/💖%20Donate-Love-ff69b4?style=for-the-badge)](https://creators.sa/m5lb)
[![Modrinth](https://img.shields.io/badge/Modrinth-Download-1bd96a?style=for-the-badge&logo=modrinth)](https://modrinth.com/plugin/relisheconomy)
...
- ⭐ **Personal Vault** — password-protected physical currency storage block with hover hologram
...
4. **(Optional)** For premium features, buy a license key from [M5LB Store](https://m5lb.run.place/?buy=relish-economy)
...
| Command | Description | Example |
|---------|-------------|---------|
| `/eco <give\|take\|set> <player\|all\|online> <amount>` | Manage balances | `/eco give Steve 1000` |
| `/eco clear <player\|all\|online> [currency]` | Zero out a balance | `/eco clear Steve coins` |
| `/eco info <player> [currency]` | View any player's balance | `/eco info Steve` |
| `/re give <player> <vault\|atm> [amount]` | Give vault or ATM block item | `/re give Steve vault` |
...
### 👤 **Player Disambiguation (Escape Wildcards)**
If a player's name conflicts with wildcards (`all`, `online`) or currency names, prefix it with `p:` or `player:` to explicitly target them (e.g., `/eco give p:all 100` or `/pay p:dollars 50`).
...
⭐ Physical currency notes and coins keep the owner name in their item metadata. `/withdraw` sets the owner to the player, crafted physical currency is rewritten to the crafting player, and custom model data can be applied per currency item. Default item display format is `{symbol} {name}` (e.g. `$ Dollars`). Default lore value line is `{symbol} {name}: {amount}`.
...
A password-protected chest block that stores physical currency items. Place a **Vault block** (Minecraft 1.21 `VAULT` material) and right-click to open.

**Vault Highlights:**
- **PIN keypad** — first open sets a PIN (4–16 digits) using click-based skull buttons — no chat required
- **Hover hologram** — look at the vault block to see stored balance(s) as a floating `TextDisplay`
- **Inventory lore** — vault item in your inventory updates to show current balance after each operation
- **Craftable** — default recipe: 8× netherite ingot surrounding 1× chest (configurable)
- **Admin give** — `/re give <player> vault [amount]` bypasses crafting

**Vault Config**
```yaml
vault:
  enabled: true
  show-hologram: true  # Show balance hologram when looking at the block
  crafting:
    enabled: true
```
...
- 🛒 **Buy a key from [M5LB Store](https://m5lb.run.place/?buy=relish-economy)**
